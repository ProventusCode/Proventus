import { integer, jsonb, serial, text, unique } from "drizzle-orm/pg-core";
...
export const contestStanding = schema.table(
  "contest_standing",
  {
  ...
  (table) => ({
    unique: unique().on(table.contestId, table.userName),
  })
);
